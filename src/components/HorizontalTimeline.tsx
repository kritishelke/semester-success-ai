...
const HorizontalTimeline = ({ roadmapData }: HorizontalTimelineProps) => {
  ...
  const leftPct = (i: number) => (count > 1 ? `${(i / (count - 1)) * 100}%` : "50%");
      ...
      <div className="mx-auto max-w-7xl px-6">
        {roadmapData && (
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-foreground mb-4">Your Personalized Career Roadmap</h2>
            ...
            <div className="flex justify-center gap-2 mt-4 flex-wrap">
              ...
              {roadmapData.preProfessional && roadmapData.preProfessional !== "None" && (
        ...
        <section className="relative mt-8 px-8 isolate overflow-visible min-h-[460px]">
          ...
          <div
            className="pointer-events-none absolute inset-x-8 top-1/2 -translate-y-1/2 h-[3px] rounded-full bg-gradient-to-r from-primary via-secondary to-accent z-0"
          />
            ...
            {roadmapSemesters.slice(0, 4).map((semester, index) => {
              ...
              return (
                <div key={index} className="absolute top-0 h-full w-64 -translate-x-1/2" style={{ left: leftPct(index) }}>
                  ...
                  <div
                    className={`absolute left-1/2 -translate-x-[1px] w-[2px] bg-border z-30 ${isAbove ? "bottom-1/2 h-2" : "top-1/2 h-2"}`}
                  />

                  {/* Card container – attached to the line */}
                  <div
                    className={`absolute left-1/2 -translate-x-1/2 w-64 ${isAbove ? "bottom-1/2 -mb-[2px]" : "top-1/2 -mt-[2px]"}`}
                  >
                    <Card
                    ...
                    >
                      <CardHeader className="text-center p-4">
                        <div className="flex flex-col items-center gap-2 mb-2">
                          ...
                          {(semester.status === "current" || semester.status === "upcoming") && (
                            <ProgressRing progress={getSemesterProgress(semester.tasks)} size="sm" showPercentage={false} />
                          )}
                        </div>
                      ...
                      <CardContent className={`${semester.status === "future" ? "relative p-4" : "p-4"}`}>
                          ...
                          <div className="text-center">
                            ...
                            >
                              {expandedSemester === index ? (
                                <>
                                  <ChevronUp className="w-3 h-3 mr-1" /> Hide Tasks
                                </>
                              ) : (
                                <>
                                  <ChevronDown className="w-3 h-3 mr-1" /> View Tasks
                                </>
                              )}
                            </Button>
                          </div>
                        ...
                        <CardContent className="p-4">
                          <div className="space-y-3">
                            {(["critical", "high", "medium", "low"] as const).map((priority) => {
                              const group = semester.tasks.filter((t: Task) => t.priority === priority);
                                  ...
                                  <div className="ml-6 space-y-1">
                                    {group.map((task: Task) => (
                                      <div key={task.id} className="flex items-center gap-2">
                                        ...
                                        />
                                        <span
                                          className={`text-xs ${getTaskCompletion(task.id) ? "line-through text-muted-foreground" : "text-foreground"}`}
                                        >
                                          {task.title}
                                        </span>
                                        {task.isPremium && <Crown className="w-3 h-3 text-primary" />}
                                      </div>
                                    ))}
                                  </div>
                                </div>
        ...
        <div className="mt-12 text-center">
          <p className="text-muted-foreground text-lg">💡 Click on "Current" or "Next" semester cards to view detailed tasks and progress</p>
        </div>
        ...
        {/* Premium Modal */}
        <PremiumModal isOpen={showPremiumModal} onClose={() => setShowPremiumModal(false)} trigger={premiumModalTrigger} />
          ...
          <Card className="bg-gradient-to-r from-primary/5 to-secondary/5 border-primary/20">
            <CardHeader className="text-center">
              <CardTitle className="text-2xl flex items-center justify-center gap-2">
                <Users className="w-6 h-6 text-primary" /> Connect with Industry Experts
              </CardTitle>
              <CardDescription>Get guidance from professionals who've walked your path</CardDescription>
            </CardHeader>
            <CardContent>
